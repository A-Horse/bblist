...
export function KanbanOverviewPanel({ kanban }: Props) {
  ...
  const recentlyIssues = useSelector((state: RootState) => selectKanbanRecentlyIssues(state, kanban.id));
  ...
  return <div>
    ...
    <div>
      ...
      {recentlyIssues.map(issue => <KanbanIssue key={issue.id} issue={issue}  kanbanId={kanban.id} onClick={() => {}}/>)}
