...
export function Overview() {
  ...
  const kanbans = useSelector((state: RootState) => selectKanbans(state, projectId));
  const isLoadingKanbans = useSelector((state: RootState) => state.project.loadingKanbans);
  ...
  return (
    <div>
...

      {
        !isShowNoKanbanGuide && (
          <div>
            {kanbans.map((kanban) => <KanbanOverviewPanel key={kanban.id} kanban={kanban} />)}
          </div>
        )
      }
    </div>
