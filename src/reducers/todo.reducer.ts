...
export function todo(
  ...
  switch (action.type) {
    case Actions.ADD_TODO.SUCCESS:
      ...
      return state.update('todoEntities', (todoEntities: any) => todoEntities.set(String(addedTodo.id), fromJS(addedTodo))).update('todoIds', (todoIds: any) => todoIds.push(String(addedTodo.id)));
    ...
    case Actions.GET_TODOLIST.SUCCESS:
      ...
      return state.update('todoIds', () => List(normalizedTodos.result)).update('todoEntities', () => fromJS(normalizedTodos.entities.Todo || {}));

    case Actions.UPDATE_TODO.SUCCESS:
      return state.updateIn(['todoEntities', String(action.payload.id)], () => fromJS(action.payload));
    ...
    case Actions.ADD_TODOBOX.SUCCESS:
      ...
      return state.update('todoBoxEntities', (todoBoxEntities: any) => todoBoxEntities.merge(normalizedAddedTodoBox.entities.TodoBox)).update('todoBoxIds', (todoBoxIds: any) => todoBoxIds.push(normalizedAddedTodoBox.result));

    case Actions.GET_TODOBOXS.SUCCESS:
      const normalizeTodoBox = normalize(action.payload, TDBoxs);
      return state.update('todoBoxIds', () => List(normalizeTodoBox.result)).update('todoBoxEntities', () => fromJS(normalizeTodoBox.entities.TodoBox || {}));
